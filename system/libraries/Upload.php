...
class CI_Upload {
	 ...
	 * @return	bool
	 */
	public function do_upload_multiple($field = 'userfile', $i)
	{
	...
	// Is $_FILES[$field] set? If not, no reason to continue.
		...
		$this->_file_mime_type_multiple($_FILES[$field], $i);
	 ...
	 * @return	void
	 */
	protected function _file_mime_type_multiple($file, $i)
	{
		// We'll need this to validate the MIME info string (e.g. text/plain; charset=us-ascii)
		$regexp = '/^([a-z\-]+\/[a-z0-9\-\.\+]+)(;\s.+)?$/';
		...
		if (DIRECTORY_SEPARATOR !== '\\')
		{
			$cmd = 'file --brief --mime ' . escapeshellarg($file['tmp_name'][$i]) . ' 2>&1';
